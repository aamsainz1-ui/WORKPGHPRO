...
interface SidebarProps {
  activeTab: 'dashboard' | 'history' | 'leave' | 'profile' | 'organization' | 'announcements' | 'admin' | 'calendar' | 'mkt' | 'payroll' | 'permissions' | 'teams' | 'meeting';
  setActiveTab: (tab: 'dashboard' | 'history' | 'leave' | 'profile' | 'organization' | 'announcements' | 'admin' | 'calendar' | 'mkt' | 'payroll' | 'permissions' | 'teams' | 'meeting') => void;
  user: UserProfile;
  lang: Language;
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, user, lang, rolePermissions }) => {
  ...
  const menuItems = [
    ...
    {
      id: 'meeting', label: lang === Language.TH ? 'ห้องประชุม' : 'Meeting', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <nav className="space-y-2">
              {filterMenuByPermissions(menuItems, user.role, rolePermissions).map((item) => (
                ...
                >
                  ...
                  <span className="text-xs uppercase tracking-normal">{item.label}</span>
                </button>
              ))}
            </nav>
          </div>
          ...
          {isAdmin && (
            <div className="space-y-2">
              ...
              >
                ...
                <span className="text-xs uppercase tracking-normal">Admin Hub</span>
              </button>
              ...
              >
                ...
                <span className="text-xs uppercase tracking-normal">{lang === Language.TH ? 'ระบบเงินเดือน' : 'Payroll Hub'}</span>
              </button>
            </div>
          )}
        </div>
        ...
        <div className="mt-auto p-8 border-t border-slate-50">
          ...
          >
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[8px] text-slate-400 font-black uppercase tracking-normal truncate">{isAdmin ? 'SYSTEM ADMIN' : user.position}</p>
            </div>
          </div>
        </div>
      </aside>
